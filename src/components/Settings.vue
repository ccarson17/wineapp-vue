<template>
    <div id="profile" class="content-centered-horizontal text-default">
        ...
        <br />
        <h3>
            Profile Info (from Entra/Microsoft)
        </h3>
        <table class="content-centered-horizontal text-default mx-auto">
            <thead>
                <tr>
...
<script>
    export default {
        ...
        methods: {
            ...
            getEntraClaims() {
                // Get the first account from MSAL (Entra)
                const account = this.msalInstance.getAllAccounts()[0];
                if (!account) return [];
                // account.idTokenClaims contains the claims
                return Object.entries(account.idTokenClaims || {}).map(([claim, value]) => ({
                    claim,
        ...
        async created() {
            ...
            this.changeTheme();
            this.claims = this.getEntraClaims();
        }
    }
</script>